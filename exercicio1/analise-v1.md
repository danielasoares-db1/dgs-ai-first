...
Documento Técnico: Orçamento de Contexto para RAG no Cenário NovaTech

1. Objetivo
Analisar o orçamento de contexto para um sistema RAG considerando:

janela total de 128.000 tokens,
reserva de 2.000 tokens para instruções,
chunks médios de 500 tokens.
2. Premissas e Cálculo Teórico

2.1 Tokens úteis para contexto recuperado:
...
126.000
 tokens
128.000−2.000=126.000 tokens
2.2 Número teórico de chunks de 500 tokens:
...
​
 =252
Resultado teórico: 252 chunks.

3. Por que não usar 100% da capacidade

Mesmo que 252 chunks caibam matematicamente, usar toda a janela é tecnicamente inadequado:

É preciso reservar tokens para a pergunta do usuário, instruções dinâmicas e resposta final.
Contexto excessivo aumenta ruído e reduz a proporção de evidência realmente útil.
Custo e latência crescem de forma relevante, com retorno marginal de qualidade.
O modelo não distribui atenção de forma uniforme em contextos longos.
Em bases com conflito documental (como versões de procedimento), excesso de material pode amplificar inconsistência em vez de reduzi-la.
Conclusão prática: orçamento operacional deve ficar abaixo do máximo teórico.

4. Conceito de Lost in the Middle

Lost in the middle é a degradação de aproveitamento de conteúdo que aparece no meio de prompts longos.
Na prática:

o início e o fim do contexto tendem a receber maior atenção,
evidências críticas posicionadas no meio podem ser subutilizadas,
isso afeta precisão e consistência de respostas, mesmo sem estourar limite de tokens.
5. Impacto na Estratégia de Retrieval em RAG

O efeito de posição muda o desenho de retrieval:

Priorizar relevância sobre volume:
melhor poucos chunks altamente pertinentes do que muitos chunks medianos.
Usar recuperação em estágios:
busca ampla inicial para recall,
reranking forte para precisão,
montagem final de contexto enxuto.
Controlar o top-k dinamicamente:
perguntas simples: menor top-k,
perguntas com conflito ou múltiplas políticas: top-k moderado e curado.
Reordenar chunks no prompt:
colocar as evidências mais fortes e normativas em posições de maior atenção.
Aplicar compressão e deduplicação:
remover redundâncias,
preservar somente trechos com valor decisório.
Incluir metadados de governança:
versão, data, tipo de documento e nível de autoridade para reduzir ambiguidade.

6. Recomendação Técnica Final

Capacidade teórica: 252 chunks.
Capacidade recomendada: inferior ao teto, com margem para entrada e saída.
Estratégia vencedora em RAG: seleção rigorosa, reranking, ordenação intencional e contexto compacto.
Em termos de qualidade, “melhor contexto” supera “mais contexto”.
